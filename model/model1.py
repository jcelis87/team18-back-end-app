...
def split_image_into_cells(img, filename, num_imgs=3):
    ...
    for hc in range(number_of_cells_high):
        ...
        for wc in range(number_of_cells_wide):
            ...
            geom = get_tile_geom(img.transform, x, y, 
                        square_dim_wide, square_dim_height)
...
def split_images(filepath, num_imgs=3):
    ...
    split_image_into_cells(img, "model/split_out/output_data")
...
def create_geometries(filepath):
    ...
    The geojson files are saved in the folder "model/geometries".
    ...
    images_to_proces = os.listdir(path='model/split_out')

    for path in images_to_proces:
        if ('.tif' in path):
            geotif_to_jpeg("model/split_out/" + path)
            sub_image_jpeg = path.replace('.tif','.jpeg')
            response, text = detect_text(
                getbytesimg_from_path("model/split_out/" + sub_image_jpeg))
            words, boundings, confidence = get_text_coords(response)
            img = rasterio.open("model/split_out/" + sub_image_jpeg)
            ...
            for i in range(len(words)):
                aux_polygon = Polygon([img.xy(get_coords(boundings,i,0,"y"),
                                                get_coords(boundings,i,0,"x")),
                                       img.xy(get_coords(boundings,i,1,"y"),
                                                get_coords(boundings,i,1,"x")),
                                       img.xy(get_coords(boundings,i,2,"y"),
                                                get_coords(boundings,i,2,"x")),
                                       img.xy(get_coords(boundings,i,3,"y"),
                                                get_coords(boundings,i,3,"x")),
                                       img.xy(get_coords(boundings,i,0,"y"),
                                                get_coords(boundings,i,0,"x"))])
            ...
            name_geometry = "model/geometries/" + sub_image_jpeg.replace('.jpeg','.geojson')
            try:
...
def combine_geometries(filepath):
    """
    Takes all the geojson files in the "model/geometries" folder and combine
    ...
    geometries_to_process = os.listdir(path="model/geometries")    
    rectangles = []
    ...
    for geometry in geometries_to_process:
        if ".geojson" in geometry:
            file = gpd.read_file("model/geometries/" + geometry)
...
def get_image_corners(filepath="model/img_to_process.tif"):
    """
...
def empty_folders():
    """
    Remove all temporary output files in the "model/split_out" and
    "model/geometries" folders. It also deletes the final output file
    ...
    for folder in folders:
        files = glob.glob(f"model/{folder}/*")
        for f in files:
            os.remove(f)
    try:
...
def run_model1(filepath="model/img_to_process.tif"):
    """
